...
def test_get_station_data_for_2024(app_client, mock_stations, mock_get_station_data):
    ...
    assert data['station'] == STUTTGART_STATION[
        'id'], f"Expected station ID {STUTTGART_STATION['id']}, got {data.get('station')}"
    assert data['name'] == STUTTGART_STATION[
        'name'], f"Expected station name {STUTTGART_STATION['name']}, got {data.get('name')}"
